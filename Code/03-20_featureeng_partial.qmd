...
```{r}
fieldweather <- read.csv("../Data/fieldweatherdata.csv")
...
fe_month <- fieldweather %>%
  ...
  # Create a date string using year and day of the year
  mutate(date_chr = paste0(year, "/", sprintf("%03d", yday))) %>%  # Ensure yday is 3 digits
  mutate(date = as.Date(date_chr, format = "%Y/%j")) %>%  # Convert to Date
  
  # Extract the month and abbreviated month name
...
fe_month_sum <- fe_month %>%
  ...
  summarise(
    ...
    across(.cols = c(dayl.s, srad.wm2, tmax.c, tmin.c, vp.pa),
           .fns = mean,
           .names = "mean_{.col}"),
    
    # Calculate the sum for precipitation variable
    across(.cols = prcp.mm,
           .fns = sum,
           .names = "sum_{.col}")
  ) %>%
